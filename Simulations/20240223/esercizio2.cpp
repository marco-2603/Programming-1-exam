...
node * makeOperation(node * n1,node *n2){
 ...
 if(isFalse(n1)||isFalse(n2)){ //se è terminale false
  return makeFalse();
 }
 if(getVar(n1)==getVar(n2)){
  node * t=makeOperation(getThen(n1),getThen(n2));
  node * e=makeOperation(getElse(n1),getElse(n2));
  if(areEquivalent(t,e)){
   deleteNode(e);
   return t;
  } else{
   return makeNode(getVar(n1), t, e);
  }
 }
 if(getVar(n1)<getVar(n2)){ //se n1 è minore di n2
  return makeNode(getVar(n1),makeOperation(getThen(n1),n2),makeOperation(getElse(n1),n2));
 }
 //se è n1>n2
  return makeNode(getVar(n2),makeOperation(getThen(n2),n1),makeOperation(getElse(n2),n1));
