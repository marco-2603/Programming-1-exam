//RISCRIVI STACK (pila) INSERENDO DOPO OGNI VALORE LA SOMMA DEI VALORI PRECEDENTI DELLO STCAK
#include <iostream>
#include <cstdlib>
...
void stackOpAus(Stack *&s,Stack *&ris,int sum){
    if(isEmpty(s)){
        return;
    }
    int v=pop(s);
    sum+=v;
    stackOpAus(s,ris,sum);
    push(ris,sum);
    push(ris,v);
    push(s, v);
}
