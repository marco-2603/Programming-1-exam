//STACK (pila) S1 S2 LEGGI SENZA MODIFICARE PRENDI ALTERNATI E INSERISCI IN LISTA (classica)
//POI ORDINA LISTA IN MODO DECRESCENTE
//RICORSIVA
#include <iostream>
#include <cstdlib>
#include <cassert>
...
/* QUESTA PRIMA CREA LA LISTA COME LEGGE E POI SAREBBE DA ORDINARE  
void calcolaRec(Stack *&s1, Stack *&s2,int l, List *&cur,int i=0){
    if(i>=l){
        return;
    }
    int v1=pop(s1);
    ...
    List *nodoNew=new List;
    cur->next=nodoNew;
    cur=nodoNew;
    calcolaRec(s1,s2,l,cur,i+1);
    cur->next=nullptr;
    push(s1,v1);
    push(s2,v2);
}*/

void calcolaRec(Stack *&s1, Stack *&s2, List *&cur,int i=0){
    if(isEmpty(s1)){
        return;
    }
    int v1=pop(s1);
