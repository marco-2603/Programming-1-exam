// DA DUE LISTE L1 L2 SCRIVI UNA TERZA LISTA L3 SECONDO ALCUNI CRITERI
// RICORSIVA
#include <iostream>
#include <string>
...
int inserisci(int num,list * l){
    int ris=0;
    if(l==nullptr){
        return 0;
    }
    if(l->code==num){
        return 1+inserisci(num,l->next);;
    }else{
        return 0+inserisci(num,l->next);
    }
}
...
list* estrai(list * l1,list * l2){
    ...
    if(l1->code%2==0){
            return new list{inserisci(l1->code,l2), estrai(l1->next,l2)};
        }else{
