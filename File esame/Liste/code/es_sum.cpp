// LETTURA DI CODA SENZA MODIFICARLA E RISCRITTURA INSERENDO DOPO OGNI VALORE 
// LA SOMMA DEI VALORI PRECEDENTI DELLO STCAK
#include <iostream>
#include <cstdlib>
#include <cassert>
using namespace std;
...
void calcolaAus(Queue *&s,Queue *&ris,int &sum){
    if(isEmpty(s)){
        return;
    }
    int v=dequeue(s);
    sum+=v;
    calcolaAus(s,ris,sum);
    enqueue(ris,sum);
    sum-=v;
    enqueue(ris,v);
    enqueue(s,v);
